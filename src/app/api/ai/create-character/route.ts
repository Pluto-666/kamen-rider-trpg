...
### 规则书中支持的变身系统（有详细规则）
1. **觉醒系统**（Blade系）- 使用Undead封印卡
2. **假面骑士系统**（Kabuto系）- 使用Zecter变身
3. **记忆体驱动器**（W系）- 使用盖亚记忆体
4. **细胞驱动器**（OOO系）- 使用核心硬币
5. **战极驱动器/创世纪驱动器**（Gaim系）- 使用Lockseed
6. **天文驱动器**（Fourze系）- 使用天文开关
7. **游戏驱动器**（Ex-Aid系）- 使用卡带
8. **时间驱动器**（Zi-O系）- 使用Ridewatch

### 当玩家选择规则书中不存在的系统时（如Faiz驱动器）
**必须严格遵守以下规则**：
1. ❌ **绝对禁止AI凭空编造规则或数值**
2. ✅ 明确告知玩家"规则书中没有该系统的详细规则"
3. ✅ 提供以下选择：
   - 选择A：使用规则书中已有的系统（推荐新手）
   - 选择B：玩家自己提供数值设定，AI只负责格式化到角色卡
4. ✅ 如果玩家选择B，必须让玩家明确提供：
   - 变身后的HP数值
   - 变身后的属性加成
   - 必杀技名称和效果
...
### 示例对话
玩家："我想用Faiz驱动器"
AI："抱歉，规则书中没有Faiz驱动器的详细规则。您有以下选择：
...
**再次强调**：AI绝不编造数值或规则，所有数值必须来自规则书或玩家自己的设定！
